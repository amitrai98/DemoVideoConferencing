...
public class VideoCall extends AppCompatActivity implements EasyPermissions.PermissionCallbacks,
        ...
        private void requestPermissions() {
                ...
                if (EasyPermissions.hasPermissions(this, perms)) {
                        mSession = new Session(VideoCall.this, AppConstants.API_KEY, AppConstants.SESSION_ID);
        ...
        public void onConnected(Session session) {
                ...
                mPublisher = new Publisher(VideoCall.this, "publisher");
        ...
        public void onStreamReceived(Session session, Stream stream) {
                ...
                final Subscriber subscriber = new Subscriber(VideoCall.this, stream);
                mSession.subscribe(subscriber);
                mSubscribers.add(subscriber);
                mSubscriberStreams.put(stream, subscriber);

                int position = mSubscribers.size() - 1;
                int id = getResources().getIdentifier("subscriberview" + (new Integer(position)).toString(), "id", VideoCall.this.getPackageName());
                RelativeLayout subscriberViewContainer = (RelativeLayout) findViewById(id);

                subscriber.setStyle(BaseVideoRenderer.STYLE_VIDEO_SCALE, BaseVideoRenderer.STYLE_VIDEO_FILL);
                subscriberViewContainer.addView(subscriber.getView());

                id = getResources().getIdentifier("toggleAudioSubscriber" + (new Integer(position)).toString(), "id", VideoCall.this.getPackageName());
